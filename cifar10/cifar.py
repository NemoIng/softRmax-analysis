...
def main():
    net = Net(device, hps['num_classes'], hps['function'], hps['conservative_a'], hps['triangular']).to(device)
    ...
    for epoch in range(1, hps['epoch'] + 1):
        ...
        scheduler.step()
        with open(path + f'cifar_{hps["function"]}_train_accuracy.txt', 'a') as f:
            f.write('[epoch %d], train_accuracy is: %.5f \n' % (epoch, train_acc))
        with open(path + f'cifar_{hps["function"]}_test_accuracy.txt', 'a') as f:
