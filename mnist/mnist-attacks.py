...
def main():
    ...
    # We move the network and data to the CPU for the testing phase 
    net = Net(device2, num_classes, function, conservative_a, kernel_size)
    net.load_state_dict(torch.load(path))
    acc_per_eps = []
    acc_class_per_eps = []
    ...
    for i, eps in enumerate(testing_eps):
        ...
        acc, classes_acc = test_model_accuracy(net, adv_data[i], eps, device2)
...
def fgsm_attack(net, loss_func, images, labels, eps) :
    ...
    # We clamp the images between 0 and 1, to make sure they have the same range as the orginials
...
def bim_attack(net, loss_func, images, labels, eps):
    ...
    for i in range(bim_iters):    
        ...
        c = (b > attack_images + eps).float() * (org_images + eps) + (b <= org_images + eps).float() * b

        images = torch.clamp(c, 0, 1).detach()

    noise = torch.clamp(images - org_images, 0, 1)
    noise = noise.cpu().detach().numpy()
    return images, noise

def test_model_accuracy(net, adv_data, eps, device2):
    net = net.to(device2)
    if not train_all:
...
def plot_acc_graph(acc_per_eps, testing_eps):
    ...
    plt.suptitle(f'{function} (MNIST {num_classes})')
    plt.xlabel("Epsilon")
    plt.ylabel("Accuracy")
    plt.grid(True)
